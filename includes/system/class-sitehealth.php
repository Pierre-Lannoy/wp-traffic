...
class Sitehealth {
	...
	public static function perfopsone_test_objectcache_do() {
		...
		if ( 'apcu' === $analytics['type'] ) {
			return [
				...
				'description' => sprintf( '<p>%s %s</p>', esc_html__( 'APCu is available on your site, but only PerfOps One suite and some few other plugins know how to take advantage of it.', 'traffic' ), sprintf( esc_html__( 'You should consider using %s to improve your site\'s speed.', 'traffic' ), '<a href="https://perfops.one/apcu-manager/">APCu Manager</a>' ) ),
				'actions'     => '',
				'test'        => $key,
			];
		}
		...
		return [
			...
			'description' => sprintf( '<p>%s %s</p>', esc_html__( 'Your site uses database transient.', 'traffic' ), esc_html__( 'You should consider using a dedicated object caching mechanism, like APCu, Memcached or Redis, to improve your site\'s speed.', 'traffic' ) ),
			'actions'     => '',
			'test'        => $key,
		];
	}
	...
	public static function perfopsone_test_shmop( $tests ) {
		...
		if ( ! array_key_exists( $key, $tests['direct'] ) ) {
			$tests['direct'][ $key ] = [
				'label' => __( 'Shared Memory Test', 'traffic' ),
	...
	public static function perfopsone_test_shmop_do() {
		...
		if ( function_exists( 'shmop_open' ) && function_exists( 'shmop_read' ) && function_exists( 'shmop_write' ) && function_exists( 'shmop_delete' ) ) {
			$result = [
				'label'       => esc_html__( 'Your site can use shared memory', 'traffic' ),
				'status'      => 'good',
				'badge'       => [
					'label' => esc_html__( 'Performance', 'traffic' ),
					'color' => 'blue',
				],
				'description' => sprintf( '<p>%s</p>', esc_html__( 'Your site can use shared memory to allow inter-process communication. That\'s great.', 'traffic' ) ),
				'actions'     => '',
				'test'        => $key,
			];
		} else {
			$result = [
				'label'       => esc_html__( 'You should allow inter-process communication', 'traffic' ),
				'status'      => 'recommended',
				'badge'       => [
					'label' => esc_html__( 'Performance', 'traffic' ),
					'color' => 'gray',
				],
				'description' => sprintf( '<p>%s</p>', esc_html__( 'You should consider using shared memory (PHP shmop) to allow inter-process communication.', 'traffic' ) ),
				'actions'     => '',
				'test'        => $key,
			];
		}
